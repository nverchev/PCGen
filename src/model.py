...
#     settings = {}
#
#     def __init__(self, input_points, z_dim, codebook, n_pseudo_inputs):
#         super().__init__()
#         self.z_dim = z_dim
#         self.codebook = codebook
#         self.dim_codes, self.book_size, self.embedding_dim = codebook.data.size()
#         self.encoder = WEncoder(input_points, z_dim, embedding_dim=self.embedding_dim)
#         self.decoder = WDecoder(w_dim, z_dim, embedding_dim=self.embedding_dim, book_size=self.book_size)
...
#         self.inference1 = nn.Linear(w_dim, z_dim)
#         self.inference2 = nn.Linear(w_dim + z_dim // 2,  z_dim)
...
class VAECW(nn.Module):
    ...
    def __init__(self, w_dim, z_dim, codebook, vae_n_pseudo_inputs, vae_dropout, **other_settings):
        super().__init__()
        self.z_dim = z_dim
        self.codebook = codebook
        self.dim_codes, self.book_size, self.embedding_dim = codebook.data.size()
...
class VQVAE(AE):
    ...
    def random_sampling(self, batch_size, initial_sampling=None, viz_att=None):
        ...
        with self.double_encoding:
            out = self.decode(data, initial_sampling, viz_att)
        return out
