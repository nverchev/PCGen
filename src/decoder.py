    ...
    def forward(self, z, s=None):
        ...
        for group in range(self.num_groups):
            ...
            if x_old is not None:
                keys = (x_group - x_group.mean(dim=2, keepdim=True)) / (x_group.std(dim=2, keepdim=True) + 0.00001)
                queries = (x_old - x_old.mean(dim=2, keepdim=True)) / (x_old.std(dim=2, keepdim=True) + 0.00001)
