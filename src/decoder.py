...
#             self.att.append(PointsConvLayer(self.h_dim[-1], 1, act=None))
#
#     def forward(self, z, s=None):
#         batch = z.size()[0]
#         device = z.device
...
#             x_att = self.att1(x_group)
#             x_group = self.group_final[group](x_group)
#             xs.append(x_group)
#         torch.cat(xs, dim=1).contiguous().detach()
#         keys = self.att2(x_att.max(2)[0])
#         x_att = x_att * torch.sigmoid(keys).unsqueeze(2)
#         att = torch.softmax, dim = 1).transpose(2, 1)
#         x_final = []
