...
class Trainer(metaclass=ABCMeta):
    ...
    def _run_session(self, partition='train', save_outputs=0):
        ...
        for batch_idx, (inputs, targets, indices) in iterable:
            ...
            if torch.is_inference_mode_enabled():
                ...
                assert not torch.isinf(criterion), print('Infs found:', outputs)
    ...
    def load(self, epoch=None):
        ...
        torch.save(self.optimizer.state_dict(), "temp_optim.pt")
...
class AETrainer(Trainer):
    ...
    def evaluate_generated_set(self, m, repeat_chamfer=10, repeat_emd=0, batch_size=64, oracle=False):
        test_dataset = []
        ...
        for _ in range(max(repeat_chamfer, repeat_emd)):
            ...
            if oracle:
                ...
                self.model.eval()
                for batch in [batch_size] * (test_l // batch_size) + [test_l % batch_size] * bool(test_l % batch_size):
                    with torch.no_grad():
    ...
    @staticmethod
    def print_statistics(name, list):
        if not list:
            return
        a = np.array(list)
        print(name)
        print(f'Number of tests: {len(a):} Min: {min(a):.4e} Max: {max(a):.4e} Mean: {a.mean():.4e} Std: {a.std():.4e}')
